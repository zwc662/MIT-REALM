...
class F16GCASFloorCeilDisturbed(F16GCASFloorCeil):
    def __init__(self):
        ...
        self.l1_control_info = {'z': 0, 'control': 0, 'hist_controls': None}
    ...
    def get_nxt_state(self, state, control):
    ...
    def linearization(self, state: State, control: Control):
        control = self.discr_to_cts(control)
        self.chk_x(state)
        self.chk_u(control) 
        state_new_fn = lambda state_control: \
            self.get_nxt_state(state_control['state'], \
                            state_control['control']
            ) #+ task.l1_control_rec.get('control', 0))
        return jacrev(state_new_fn)({'state': state, 'control': control})
        
    @override
    def step(self, state: State, control: Control) -> State:
        cts_control = self.discr_to_cts(control)
        self.chk_x(state)
        self.chk_u(cts_control) 
        # Integrate using RK4.
        nxt_state = self.get_nxt_state(state, cts_control + self.l1_control_info.get('control'))
        state_new = self.get_dist_state(nxt_state, state, control)
        return state_new 
    ...
    def get_dist_state(self, nxt_state: State, state: State, control: Control) -> State: 
        theta_err = self.err_fn(nxt_state[F16Safeguarded.THETA])
        dist_nxt_state = nxt_state.at[F16Safeguarded.THETA].add(theta_err)
        return dist_nxt_state 
