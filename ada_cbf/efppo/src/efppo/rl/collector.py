...
def collect_single_env(
    ...
    def _body(state: CollectorState, key):
        obs_pol = task.get_obs(state.state[env_idx])
        a_pol: tfd.Distribution = get_pol(obs_pol, state.z[env_idx])
        control, logprob = a_pol.experimental_sample_and_log_prob(seed=key)
        envstate_new = task.step(state.state[env_idx], np.asarray(control))

        # Z dynamics.
        l = task.l(envstate_new, control)
        z_new = (state.z[env_idx:env_idx+1] - l) / disc_gamma
        z_new = jnp.clip(z_new, z_min, z_max)

        new_state = CollectorState(state.steps[env_idx:env_idx+1], envstate_new.reshape(1, -1), z_new)
        return new_state, (envstate_new, obs_pol, z_new, control, logprob)

    # Randomly sample z0.
    key_z0, key_step = jr.split(key0)
    z0 = jr.uniform(key_z0, minval=z_min, maxval=z_max)
    colstate0.z.at[env_idx].set(z0)
    ...
    collect_state.steps.at[env_idx].set(collect_state.steps[env_idx] + rollout_T)

    obs_final = task.get_obs(collect_state.state[env_idx])

    # Add the initial observations.
    Tp1_state = jnp.stack((colstate0.state[env_idx], *T_envstate))
    T_state_fr, T_state_to = Tp1_state[:-1], Tp1_state[1:]
    Tp1_obs = jnp.stack((*T_obs, obs_final))
    Tp1_z = jnp.stack((jnp.asarray([z0]), *T_z))
    T_l = []
    Th_h = []
...
class Collector(struct.PyTreeNode):
    ...
    def collect_batch_iteratively(
        ...
        key_pol, key_reset_bernoulli, key_reset = jr.split(key0, 3)
        b_keys = jr.split(key_pol, self.cfg.n_envs)
        collect_fn = ft.partial(self._collect_single_env, get_pol=get_pol, disc_gamma=disc_gamma, z_min=z_min, z_max=z_max)

        # Initialize lists to accumulate results
        bT_outputs = []
        # Use a for loop instead of jax.vmap
        for i in range(self.cfg.n_envs):
            collect_state, bT_output = self._collect_single_env(
                i, 
                b_keys[i], 
                self.collect_state, 
                get_pol=get_pol, 
        ...
        # Resample x0
        b_shouldreset = jr.bernoulli(key_reset_bernoulli, self.p_reset, shape=(self.cfg.n_envs,))
        b_shouldreset = jnp.logical_or(b_shouldreset, self.collect_state.steps >= self.cfg.max_T)
        b_shouldreset = b_shouldreset | jax.vmap(self.task.should_reset)(self.collect_state.state)
        b_state_reset = self.task.sample_x0_train(key_reset, self.cfg.n_envs)
        ...
        # Again, use a for loop instead of jax.vmap
        b_steps_new = []
        b_state_new = []
        for i in range(self.cfg.n_envs):
            steps_new, state_new = reset_fn(
                b_shouldreset[i], b_state_reset[i], self.collect_state.state[i], self.collect_state.steps[i]
            )
            b_steps_new.append(steps_new)
            b_state_new.append(state_new)

        # Stack the results
        b_steps_new = jnp.stack(b_steps_new)
        b_state_new = jnp.stack(b_state_new)
        collect_state = CollectorState(b_steps_new, b_state_new, collect_state.z)
