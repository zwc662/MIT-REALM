...
from efppo.networks.network_utils import ActLiteral, HidSizes, get_act_from_str, get_default_tx, rsample, anti_rsample
from efppo.networks.policy_net import DiscretePolicyNet, EnsembleDiscretePolicyNet, ContinuousPolicyNet
from efppo.networks.train_state import TrainState 
from efppo.networks.critic_net import (
    DiscreteCriticNet, EnsembleDiscreteCriticNet, 
    #DiscreteCriticStdNet, EnsembleDiscreteCriticStdNet,
    ContinuousCriticNet, EnsembleContinuousCriticNet
)
...
from efppo.task.dyn_types import (
    Control, BControl, BBControl, ZBBControl,
    HFloat, BHFloat, BBHFloat, ZBBHFloat,
    Obs, BObs, BTObs,
    LFloat, BLFloat, ZBLFloat, 
    HFloat, BHFloat, BTHFloat, BBTHFloat,
    ZFloat, ZBFloat, 
    ZBBFloat, ZBTState, ZFloat
)
...
class Baseline(Generic[_Algo], struct.PyTreeNode):
    ...
    def get_mode_and_prob(self, obs, z):
        dist = self.policy.apply(self.standardize(obs), z)
...
class BaselineSAC(Baseline):
    ...
    def update_critic(self, batch: Baseline.Batch) -> tuple["BaselineSAC", dict]:
        ...
        def get_critic_loss(params):
            b_critics = jax.vmap(ft.partial(self.critic.apply_with, params=params))(self.standardize(batch.b_obs), batch.b_z, batch.b_control).reshape(-1, self.cfg.net.n_critics)
    ...
    def update_policy(self, batch: Baseline.Batch) -> tuple["BaselineSAC", dict]:
        ...
        def get_pol_loss(pol_params):
            ...
            sac_loss = jnp.mean(b_logprobs * self.temp - b_critic) 
    ...
    def eval_single_z(self, task: Task, z: float, rollout_T: int, contour_size: Tuple[int]):
        ...
        bb_control, bb_prob = jax_vmap(self.get_mode_and_prob, rep=2)(bb_obs, bb_z)
        bb_critic = jax_vmap(self.critic.apply, rep=2)(self.standardize(bb_obs), bb_z, bb_control)
        bb_target_critic = jax_vmap(self.get_target_critic, rep=2)(bb_obs, bb_z, bb_control)
...
class BaselineSACDisc(Baseline):
    ...
    def update_critic(self, key: PRNGKey, batch: Baseline.Batch, critic: TrainState, target_critic: TrainState) -> tuple["TrainState", "TrainState", dict]:
        ...
        if self.cfg.net.n_critics > 2:
            ...
            # Compute SVGD updates
            ensemble_critic_grads = svgd_update(ensemble_critic_grads, kernel_matrix, kernel_gradients)
            critic_grads['EnsembleDiscreteCriticNet_0'] = ensemble_critic_grads
...
class BaselineDQN(Baseline):
    ...
    def get_critic(self, obs, z):
        return self.critic.apply(obs, z)
        critic = self.critic.apply(obs, z) 
        return critic.max()

    def get_target_critic(self, obs, z):
        return self.target_critic.apply(obs, z) 
        target_critic = self.target_critic.apply(obs, z) 
        return target_critic.max()
...
class BaselineDQNCBF(BaselineDQN):
    @classmethod
    def create(cls, key: jr.PRNGKey, task: Task, cfg: BaselineCfg):
        baseline = super(BaselineDQNCBF, cls).create(key, task, cfg)
        base_kwargs = {k: getattr(baseline, k) for k in baseline.__dataclass_fields__}
        
        return BaselineDQNCBF(**base_kwargs)
    ...
    def sample_action(self, obs_pol, z, key):
        critic = self.critic.apply(self.standardize(obs_pol), z).mean(axis = -2)
        a_pol = tfd.Categorical(logits = critic)
        control, logprob = a_pol.experimental_sample_and_log_prob(seed=self.key)
        return control, logprob
    
    def get_mode_and_prob(self, obs, z):
        critic = self.critic.apply(self.standardize(obs), z).mean(axis = -2)
        dist = tfd.Categorical(logits = critic)
        mode_sample = dist.mode()
        mode_prob = jnp.exp(dist.log_prob(mode_sample))
        return mode_sample, mode_prob
    ...
    def update_critic(self, batch: Baseline.Batch) -> tuple["BaselineDQN", dict]:
        ...
        b_target_critic = b_nxt_critic 
        b_target_critic -= jax.nn.relu(b_target_critic) * batch.b_done 
        b_target_critic =  jnp.maximum(b_target_critic, - batch.b_l)
...
class BaselineDGQNCBF(BaselineDQNCBF):
    @classmethod
    def create(cls, key: jr.PRNGKey, task: Task, cfg: BaselineCfg):
        baseline = super(BaselineDGQNCBF, cls).create(key, task, cfg)
        base_kwargs = {k: getattr(baseline, k) for k in baseline.__dataclass_fields__}

        
        return BaselineDGQNCBF(**base_kwargs)
