...
def check_nan(grad: _PyTree) -> _PyTree:
    return jnp.any(jnp.asarray([jnp.any(jnp.isnan(x)) for x in jtu.tree_leaves(grad)]))

def compute_norm_and_clip(grad: _PyTree, max_norm: float) -> tuple[_PyTree, FloatScalar]:
    has_nan = check_nan(grad)
    g_norm = compute_norm(grad)
    clipped_g_norm = jnp.maximum(max_norm, g_norm)
    clipped_grad = jtu.tree_map(lambda t: (t / clipped_g_norm) * max_norm * has_nan, grad) 

    return clipped_grad, g_norm * has_nan
