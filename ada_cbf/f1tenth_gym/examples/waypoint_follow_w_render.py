...
class PurePursuitPlanner:
    ...
    def __init__(self, conf, wb):
        ...
        self.lookahead_points = self.waypoints[np.asarray([0, 0])]
    ...
    def render_lookahead_points(self, GL_QUADS, e):
        scaled_points = self.lookahead_points
        print(scaled_points.shape)
        diffs = scaled_points[1:] - scaled_points[:-1]
        print(diffs.shape)
        angles = np.arctan2(diffs[:, 1], diffs[:, 0]).flatten().tolist()
        ...
        if not hasattr(e, "lookahead_points"):
            ...
            for i in range(scaled_points.shape[0]):
                vertices_np = 50 * get_vertices(np.array(scaled_points[i]), CAR_LENGTH / 2.0, CAR_WIDTH  / 2.0)
                ...
                e.lookahead_points.append(
                    e.batch.add(
                        ...
                        None, 
                        ('v2f', vertices), 
                        ('c3B', (np.asarray([172, 97, 185, 172, 97, 185, 172, 97, 185, 172, 97, 185]) - (i + 1) * scaled_points.shape[0]).tolist())
                    )
                )
        else: 
            for i in range(scaled_points.shape[0]):
    ...
    def plan(self, pose_x, pose_y, pose_theta, lookahead_distance, vgain):
        ...
        nearest_point_ind, lookahead_point_ind = self._get_current_waypoint(self.waypoints, lookahead_distance, position, pose_theta)
        if lookahead_point_ind > nearest_point_ind:
            self.lookahead_points = self.waypoints[nearest_point_ind: lookahead_point_ind + 1]
        elif lookahead_point_ind < nearest_point_ind:
            self.lookahead_points = np.concatenate((self.waypoints[nearest_point_ind:], self.waypoints[:lookahead_point_ind + 1]), axis = 0)
        else:
            self.lookahead_points = self.waypoints[nearest_point_ind: nearest_point_ind + 1]
        self.lookahead_points = self.lookahead_points[np.asarray([0, -1])]
        if self.lookahead_points is None:
            return 4.0, 0.0

        speed, steering_angle = get_actuation(pose_theta, self.lookahead_points[-1], position, lookahead_distance, self.wheelbase)
        speed = vgain * speed
...
def main():
    ...
    def render_callback(env_renderer):
        ...
        planner.render_lookahead_points(GL_QUADS, env_renderer)
