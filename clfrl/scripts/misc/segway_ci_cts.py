...
def main():
    set_logger_format()
    plot_dir = mkdir(get_script_plot_dir() / "segway" / "cts")
    task = Segway()

    # x0 = np.array([0.0, -0.5, 0.0, 0.0])
    # x0 = np.array([0.0, 0.5, 0.0, -5.0])
    x0 = np.array([0.0, -1.0, 0.0, 5.0])
    # x0 = np.array([0.0, 0.0, -5.1, 0.0])
    tf = 150.0 * task.dt
    ...
    for ii, setup in enumerate(task.phase2d_setups()):
        ...
        def get_Th(state0):
            sim = SimCtsReal(task, task.nom_pol_lqr, tf, result_dt=task.dt)
        ...
        figsize = (2 * 5, 2)
        fig, axes = plt.subplots(1, 5, figsize=figsize, layout="constrained")
