...
import run_config.ncbf_pol.doubleintwall_cfg
import wandb
from clfrl.dyn.doubleint_wall import DoubleIntWall
from clfrl.ncbf_pol.ncbf_pol import NCBFPol
...
def main(name: str = typer.Option(..., help="Name of the run."), group: str = typer.Option(None), seed: int = 7957821):
    ...
    CFG = run_config.ncbf_pol.doubleintwall_cfg.get(seed)

    nom_pol = task.nom_pol_osc
    CFG.extras = {"nom_pol": "osc"}

    alg: NCBFPol = NCBFPol.create(seed, task, CFG.alg_cfg, nom_pol)
    CFG.extras = {"nom_pol": "random"}

    loss_weights = {"Loss/Unsafe": 10.0, "Loss/Safe": 10.0, "Loss/Descent": 0.01, "Loss/Nominal": 1e-5}
    CFG.extras["loss_weights"] = loss_weights

    run_dir = init_wandb_and_get_run_dir(CFG, "ncbf_pol", "ncbf_pol_dbint", name, group=group)
    ...
    for idx in range(LCFG.n_iters + 1):
        ...
        if should_eval:
            eval_data: NCBFPol.EvalData = jax2np(alg.eval())
            ...
            plotter.V_div(bb_Xs, bb_Ys, eval_data.bb_V, f"V/V_{suffix}")
            plotter.V_div(bb_Xs, bb_Ys, eval_data.bb_Vdot, f"Vdot/Vdot_{suffix}")
