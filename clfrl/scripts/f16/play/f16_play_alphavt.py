...
def gen():
    ...
    # Save results.
    npz_path = plot_dir / "play_alphavt.npz"
...
def plot():
    ...
    nom = np.array([0.1, 425.0])
    # nom = np.array([0.2, 400.0])
    ...
    ax.plot(T_x[0, task.ALPHA], T_x[0, task.VT], marker="o", color="C5", ms=1.0, zorder=10)
    task.plot_alphavt(ax)
    cbar = fig.colorbar(cs, ax=ax, shrink=0.8)
    fig.savefig(plot_dir / "alphavt.pdf")
    plt.close(fig)
    ...
    fig.savefig(plot_dir / "alphavt_traj.pdf")
    plt.close(fig)
    ...
    task.plot_alphavt(ax)
    cbar = fig.colorbar(cs, ax=ax, shrink=0.8)
    fig.savefig(plot_dir / "alphavt_ttc.pdf")
    plt.close(fig)
    ###################################################
