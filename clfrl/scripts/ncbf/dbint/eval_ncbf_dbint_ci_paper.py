...
def main(ckpt_path: pathlib.Path):
    ...
    CFG = run_config.ncbf.doubleintwall_cfg.get(0)
    nom_pol = task.nom_pol_osc
    alg: NCBF = NCBF.create(0, task, CFG.alg_cfg, nom_pol)
    ...
    pol = ft.partial(alg.get_cbf_control, 2.0)
    ...
    bbT_Vh = jax2np(jax_jit(rep_vmap(alg.get_V, rep=3))(bbT_x))

    bbT_u = np.zeros(0)

    ci_data = CIData("NCBF", task.name, setup_idx, bbT_x, bbT_u, bbTh_h, bb_Xs, bb_Ys, notes={"bbT_Vh": bbT_Vh})
