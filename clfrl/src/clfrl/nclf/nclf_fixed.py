...
from clfrl.dyn.dyn_types import BBControl, BState, BTState, State
from clfrl.dyn.task import Task
from clfrl.nclf.min_norm_control import (
    min_norm_control,
    min_norm_control_fixed,
    min_norm_control_interp,
    min_norm_control_minrate,
)
from clfrl.nclf.sim_nclf import SimNCLF
from clfrl.networks.mlp import MLP
from clfrl.networks.nclf import NormSqMetricFn, NormSqMetricFn2
...
class NCLFFixedCfg:
    ...
    hids: HidSizes
    train_cfg: NCLFFixedTrainCfg
    eval_cfg: NCLFFixedEvalCfg
...
class NCLFFixed(struct.PyTreeNode):
    collect_idx: int
    key: PRNGKey
    V: TrainState[FloatScalar]
    Ql: TrainState[tfd.Distribution]
    pol: TrainState[tfd.Distribution]

    cfg: NCLFFixedCfg = struct.field(pytree_node=False)
    ...
    @classmethod
    def create(
        cls, seed: int, task: Task, cfg: NCLFFixedCfg, Ql: TrainState = None, pol: TrainState[tfd.Distribution] = None
    ) -> "NCLFFixed":
        ...
        return NCLFFixed(jnp.array(0, dtype=jnp.int32), key, V, Ql, pol, cfg, task)
    ...
    @ft.partial(jax.jit, donate_argnums=0)
    def update_warmup(self, b_x: BState):
        def loss_fn(params):
            # L2 on Ql to learn an approximate shape.
            dist = self.pol.apply(b_polobs)
            b_control = dist.mode()
            ebl_Ql = self.Ql.apply(b_Vobs, b_control)
            ...
            loss_warmup = jnp.mean((b_Ql - b_V) ** 2)
            info_dict = {"Loss/Warmup": loss_warmup}

            return loss_warmup, info_dict

        b_Vobs, b_polobs = jax_vmap(self.task.get_obs)(b_x)
    ...
    def update(self, b_x: BState, loss_weights: MetricsDict):
        def loss_fn(params):
            ...
            b_u = jax_vmap(self.get_nom_control)(b_x)
    ...
    def get_nom_control(self, state: State):
        Vobs, polobs = self.task.get_obs(state)
        dist = self.pol.apply(polobs)
        return dist.mode()

    def get_control_minrate(self, lambd: float, state: State):
        u_lb, u_ub = np.array([-1.0]), np.array([+1.0])
        V_apply = ft.partial(self.get_V, self.V.params)
        ...
        f = self.task.f(state)
        G = self.task.G(state)
        u, _, _ = min_norm_control_minrate(self.desc_rate, lambd, u_lb, u_ub, V, Vx, f, G)
        return self.task.chk_u(u)

    @ft.partial(jax.jit, static_argnames=["interp"])
    def eval(self, interp: float = 1.0) -> EvalData:
        ...
        sim = SimNCLF(self.task, ft.partial(self.get_control_minrate, interp), self.cfg.eval_cfg.eval_rollout_T)
