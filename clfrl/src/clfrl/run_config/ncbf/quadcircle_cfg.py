...
def get(seed: int) -> RunCfg[NCBFCfg, SACLoopCfg]:
    lam = 1.0
    train_cfg = NCBFTrainCfg(batch_size=8192, lam=lam, unsafe_eps=0.1, safe_eps=0.1)
    eval_cfg = NCBFEvalCfg(eval_rollout_T=64)
    alg_cfg = NCBFCfg(act="tanh", lr=Constant(3e-4), hids=[256, 256, 256], train_cfg=train_cfg, eval_cfg=eval_cfg)
    loop_cfg = SACLoopCfg(n_iters=500_000, ckpt_every=5_000, log_every=100)
