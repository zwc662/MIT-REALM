...
from clfrl.dyn.dyn_types import HFloat, THFloat
from clfrl.utils.jax_types import FloatScalar, TFloat
...
def compute_disc_int_trap(lambd: FloatScalar, dt: FloatScalar, Th_h: THFloat) -> THFloat:
    """Compute discretized integral using trapezoidal integration."""
    T, nh = Th_h.shape
    gamma = jnp.exp(-lambd * dt)

    T_ts = jnp.arange(T) * dt
    Tm1_tl = T_ts[:-1]
    Tm1_tr = T_ts[1:]
    ...
    Tm1h_hl = Th_h[:-1, :]
    Tm1h_hr = Th_h[1:, :]
    Tm1_coeff_l = lambd * jnp.exp(-lambd * Tm1_tl[:, None])
    Tm1_coeff_r = lambd * jnp.exp(-lambd * Tm1_tr[:, None])
    Tm1h_int_terms = dt * 0.5 * (Tm1_coeff_l * Tm1h_hl + Tm1_coeff_r * Tm1h_hr)
    Tm1h_disc_int = jnp.cumsum(Tm1h_int_terms, axis=0)
    Th_disc_int = jnp.concatenate([jnp.zeros((1, nh)), Tm1h_disc_int], axis=0)

    return Th_disc_int


def compute_disc_avoid_terms(lambd: FloatScalar, dt: FloatScalar, Th_h: THFloat) -> DiscAvoidTerms:
    ...
    # # TODO: Compute the disc avoid for all timesteps, not just x0
    # Tm1h_int = jnp.cumsum(Tm1h_int_terms, axis=0)
    # assert Tm1h_int.shape == Tm1h_int_terms.shape

    # 2: Compute the left branch of the max.
    Th_terms = Th_disc_int + T_disc[:, None] * Th_h
    h_max_lhs = jnp.max(Th_terms, axis=0)

    # 3: The integral on the right branch is the last term.
    h_disc_int_rhs = Th_disc_int[-1, :]

    return DiscAvoidTerms(gamma ** (T - 1), Th_disc_int, h_max_lhs, h_disc_int_rhs)
