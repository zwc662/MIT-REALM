...
def main(ckpt_path: pathlib.Path):
    ...
    figsize = np.array([task.nh * 3.2, 3])
    fig, axes = plt.subplots(1, task.nh, figsize=figsize, layout="constrained")
    for ii, ax in enumerate(axes):
        cs0 = ax.contourf(bb_Xs, bb_Ys, bbh_LGV[:, :, ii], levels=21, norm=CenteredNorm(), cmap="RdBu_r", alpha=0.9)
        cs1 = ax.contour(bb_Xs, bb_Ys, bbh_LGV[:, :, ii], levels=[0.0], colors=[PlotStyle.ZeroColor], linewidths=0.5)
        task.plot_phase(ax)
        ax.set_title(h_labels[ii])
        cbar = fig.colorbar(cs0, ax=ax)
        cbar.add_lines(cs1)
