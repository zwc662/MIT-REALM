...
from pncbf.ncbf.stateful_dset_buffer import StatefulDsetBuffer
...
from pncbf.utils.jax_utils import jax2np, jax_default_x32, jax_jit_np, tree_any, tree_cat, tree_copy
...
def main(
    ...
    CFG = run_config.int_avoid.f16two_cfg.get(seed)
    ...
    if ckpt is not None:
        ...
        alpha_safe, alpha_unsafe = 2.0, 10.0
        nom_pol = ft.partial(alg.get_cbf_control_sloped, alpha_safe, alpha_unsafe, nom_pol=alg.nom_pol)
        CFG.extras = {"nom_pol": "ckpt_cbf"}
        CFG = run_config.int_avoid.f16two_cfg.get_pi(seed)

    alg: IntAvoid = IntAvoid.create(seed, task, CFG.alg_cfg, nom_pol)

    loss_weights = {"Loss/Vh_mse": 1.0, "Loss/Equil": 1.0}
    CFG.extras["loss_weights"] = loss_weights

    run_dir = init_wandb_and_get_run_dir(CFG, "intavoid_f16two", "intavoid_f16two", name, group=group)
    ...
    bb_V_noms = jax_jit_np(ft.partial(alg.V_for_pol, nom_pol, use_pid=False))()
    ...
    for idx in range(LCFG.n_iters + 1):
        ...
        if should_eval:
            ...
            if tree_any(lambda x: jnp.isnan(x).any(), alg.Vh):
                logger.error("NaN in Vh NN!")
                break
