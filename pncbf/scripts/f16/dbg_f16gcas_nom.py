...
from pncbf.utils.jax_utils import jax2np, jax_default_x32, jax_jit, jax_vmap
...
@app.command()
def gen(ckpt_path: pathlib.Path, do_plot: bool = True, x0: np.ndarray = None):
    jax_default_x32()
    set_logger_format()
    task = F16GCAS()
...
def plot(ckpt_path: pathlib.Path):
    ...
    for ii, ax in enumerate(axes):
        ...
        ax.axhline(0.0, color="0.4", lw=0.3)
