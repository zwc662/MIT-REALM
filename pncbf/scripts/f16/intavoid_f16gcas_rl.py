...
from pncbf.rl.int_avoid import IntAvoid, GoalConditionedIntAvoid
...
def main(name: str = typer.Option(..., help="Name of the run."), group: str = typer.Option(None), entity: str = typer.Option(None), seed: int = 7957821):
    ...
    CFG = pncbf.run_config.int_avoid.f16gcas_cfg.get(seed)
    nom_pol = task.nom_pol_pid

    alg: GoalConditionedIntAvoid = GoalConditionedIntAvoid.create(seed, task, CFG.alg_cfg, nom_pol)
     
    CFG.extras = {"nom_pol": "pid"}
 
    project = "intavoid_f16gcas_gcrl" 
    job_type = "intavoid_f16gcas_gcrl"
    ...
    for idx in range(LCFG.n_iters + 1):
        ...
        alg, loss_info = alg.update(batch)
