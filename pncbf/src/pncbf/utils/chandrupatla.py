...
from pncbf.utils.jax_utils import jax_vmap, tree_where

_F = FloatScalar


class Chandrupatla2State(NamedTuple):
...
class Chandrupatla2:
    ...
    def init_state(self, lb: FloatScalar, ub: FloatScalar) -> Chandrupatla2State:
        if self.batch_init:
    ...
    def batch_init_state(self, lb: FloatScalar, ub: FloatScalar):
        ...
        return Chandrupatla2State(x1, x2, y1, y2, jnp.array(self.init_t), jnp.array(0))

    def simple_init_state(self, lb: FloatScalar, ub: FloatScalar) -> Chandrupatla2State:
        x1, x2 = lb, ub
        y1, y2 = self.root_fn(x1), self.root_fn(x2)
        return Chandrupatla2State(x1, x2, y1, y2, jnp.array(self.init_t), jnp.array(0))

    def update_step(self, state: Chandrupatla2State, _):
        ...
        new_state = Chandrupatla2State(x1, x2, y1, y2, new_t, fail_counter)
        return new_state, (x, y, used_which)

    def _update(self, state: Chandrupatla2State, _):
    ...
    def refine_output(self, state: Chandrupatla2State) -> FloatScalar:
        # Secant approximation of the root. We can do this safely because we have tight brackets.
        x1, x2, y1, y2, t, fail_counter = state
        secant_x = (y2 * x1 - y1 * x2) / (y2 - y1)
        return jnp.where(jnp.isnan(secant_x), 0.5 * (x1 + x2), secant_x)
