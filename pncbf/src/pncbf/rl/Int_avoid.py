...
class IntAvoid(struct.PyTreeNode):
    ...
    def create(cls, seed: int, task: Task, cfg: IntAvoidCfg, nom_pol: Callable) -> "IntAvoid":
        key, key_Vh, key_Qh = jr.split(jr.PRNGKey(seed), 3)
        pol_obs, control, V_obs, z = np.zeros(task.n_polobs), np.zeros(task.nu), np.zeros(task.n_Vobs), np.array(0.0)
        act = get_act_from_str(cfg.act)


        # Encoder for z. Params not shared.
        z_base_cls = ft.partial(ZEncoder, nz=cfg.nz_enc, z_mean=cfg.net.z_mean, z_scale=cfg.net.z_scale)

        
        # Define V network.
        if cfg.new_net:
            Vh_cls = ft.partial(TmpNet, 2, cfg.hids[0], act)
            Qh_cls = ft.partial(TmpNet, 2, cfg.hids[0], act)
        else:
        ...
        if cfg.use_multi_norm:
            ...
            Qh_cls = ft.partial(MultiNormValueFn, Vh_cls, task.nh, 2 * task.nx)
        else:
            Vh_cls = ft.partial(MultiValueFn, Vh_cls, task.nh)
            Qh_cls = ft.partial(MultiValueFn, Vh_cls, task.nh)
        ...
        Vh_def = ZWrapper(Ensemble(Vh_cls, cfg.n_Vs), z_base_cls)
        Vh_tx = get_default_tx(cfg.lr.make(), cfg.wd.make())
        Vh = TrainState.create_from_def(key_Vh, Vh_def, (pol_obs,z,), Vh_tx)

        # Target network for stability.
        Vh_tgt_def = ZWrapper(Ensemble(Vh_cls, num=cfg.n_min_tgt), z_base_cls)
        ...
        Qh_def = ZWrapper(Ensemble(Qh_cls, cfg.n_Vs), z_base_cls)
        Qh_tx = get_default_tx(cfg.lr.make(), cfg.wd.make())
        Qh = TrainState.create_from_def(key_Qh, Qh_def, (pol_obs, z, control,), Qh_tx)

        # Target network for stability.
        Qh_tgt_def = ZWrapper(Ensemble(Qh_cls, num=cfg.n_min_tgt), z_base_cls)
        Qh_params_copy = tree_copy(Qh.params)  # Copy so that we can donate buffer.
        Qh_tgt = TrainState.create(apply_fn=Qh_tgt_def.apply, params=Qh_params_copy, tx=empty_grad_tx())
        ...
        lam = jnp.array(0.0)
        return IntAvoid(
            col_idx, upd_idx, key, Vh, Vh_tgt, Qh, Qh_tgt, nom_pol, cfg, task, lam, lam_sched.make(), cfg.train_cfg.tgt_rhs.make()
        )
