...
from pncbf.ncbf.compute_disc_avoid import (
    ...
    compute_disc_avoid_linexact_np,
    compute_disc_avoid_terms,
    compute_disc_avoid_trap_np,
    compute_disc_int_trap,
)
from pncbf.utils.jax_utils import jax2np, jax_use_cpu, jax_use_double, jax_vmap
...
def main():
    ...
    assert terms.Th_disc_int_lhs.shape == Th_disc_int.shape
    ...
    disc_int_trap = compute_disc_avoid_trap_np(lam, dt, Th_h)
    disc_int_linexact = compute_disc_avoid_linexact_np(lam, dt, Th_h)
    ...
    ####################################################################
    # Plot just the integral int_0^t lam exp(-lam s) h(s) ds.
    logger.info("Plotting...")
    plot_dir = get_script_plot_dir()

    fig, axes = plt.subplots(2, layout="constrained")
    axes[0].plot(T_t, terms.Th_disc_int_lhs, color="C0", label="Discr", zorder=12, lw=0.2)
    ...
    axes[0].axhline(Th_disc_int[0], color="C5", ls="--")

    print(
        "Th_disc_int[0]={}, T_int_cts[-1]={}, err={}".format(
            Th_disc_int[0], T_int_cts[-1], Th_disc_int[0] - T_int_cts[-1]
        )
    )
    print("err: trap={}, exact={}".format(disc_int_trap - T_int_cts[-1], disc_int_linexact - T_int_cts[-1]))
